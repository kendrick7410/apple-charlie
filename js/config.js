...
Game.PATHS = [
    ...
    { x: 970,  y: 1560, w: 60,   h: 900 },   // route vers le sud (jusqu'à la ferme)
    { x: 970,  y: 2400, w: 1600, h: 60  }    // route vers l'est jusqu'à la ferme
];
...
Game.getSkin = function(id) {
    if (!id || id === Game.SKIN_DEFAULT.id) return Game.SKIN_DEFAULT;
    for (var i = 0; i < Game.SKINS.length; i++) { if (Game.SKINS[i].id === id) return Game.SKINS[i]; }
    return Game.SKIN_DEFAULT;
};
